...
| `commitToStore` | `boolean` | Implies if the change should be recorded by the `store`. Recorded changes are emmitted and listened to by other components, such as `History` for undo / redo purposes. Defaults to `false`. |
